...
import type { People } from '@/stores/interfaces/people.interface'
...
const peopleToDelete = ref<number | null>(null)

const openDeleteModal = (peopleId: number) => {
...
const fetchPeople = async () => {
  ...
  loading.value = peopleStore.loading
  people.value = peopleStore.people
}

const confirmDeletePeople = async () => {
  loading.value = true
  if (peopleToDelete.value) {
    isDeleteModalOpen.value = false

    try {
      await peopleStore.deletePeople(peopleToDelete.value)
      people.value = peopleStore.people
      peopleToDelete.value = null
      loading.value = false
    } catch (error) {
      ...
      <div class="w-full mb-1">
        ...
        <div class="items-center justify-between block sm:flex">
          ...
          <div class="flex items-center ml-auto space-x-2 sm:space-x-3">
            <button
              v-on:click="router.push('/event/people/add')"
              ...
              >
                ...
                >
                  <td class="w-4 p-4">
                    <div class="flex items-center">
                      <input
                        ...
                        class="w-4 h-4 border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300 dark:focus:ring-primary-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600"
                      />
                      <label :for="'index-' + person.id" class="sr-only"> checkbox </label>
                    </div>
                  </td>
                  <td
                  ...
                  <td
                    class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white"
                  >
                    <data value="technology">{{ person.name }}</data>
                  </td>
                  <td
                    class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white"
                  >
                    <data value="technology">{{ person.birth_date }} </data>
                  </td>
                  <td
                    class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white"
                  >
                  ...
                  <td
                    class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white"
                  >
                    <data value="technology">{{ getFormatedDate(person.created_at) }}</data>
                  </td>
                  <td class="p-4 space-x-2 whitespace-nowrap">
                    <button
                      v-on:click="router.push(`/event/people/edit/${person.id}/${person.slug}`)"
                      type="button"
                    ...
                    <button
                      ...
                      @click="openDeleteModal(person.id)"
